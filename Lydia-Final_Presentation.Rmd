...
confusionMatrix(class_prediction, bc_test$class)
```

## Comparing models with different splitting criterion
```{r}
# Training gini-based model
bc_model_gini <- rpart(formula = class ~ ., data = bc_train, method = "class", parms = list(split = "gini"))

# Training information-based model 
bc_model_info <- rpart(formula = class ~ ., data = bc_train, method = "class", parms = list(split = "information"))
...
ce(actual = bc_test$class, predicted = pred_gini)
ce(actual = bc_test$class, predicted = pred_info)
```
...
# Compute AUC
auc(actual = ifelse(bc_test$class == "2", 1, 0), predicted = pred[, "2"])
```
...
acc1 <- rep(NA, k) # 1 hidden layer
acc2 <- rep(NA, k) # 2 hidden layers
acc3 <- rep(NA, k) # 3 hidden layers
```
...
for (i in seq(k)){
  ...
  accu1 <- mean(e1 == actual)
  accu2 <- mean(e2 == actual)
  accu3 <- mean(e3 == actual)
